    ...
    .directive('money', ["$parse", function ($parse) {
        function link(scope, el, attrs, ngModelCtrl) {
            ...
            function updateValuePrecision() {
                var modelValue = parseFloat(ngModelCtrl.$modelValue)||0;

                if (isValueValid(modelValue) && isPrecisionValid()) {
                    if (!(ngModelCtrl.$isEmpty(modelValue) || isUndefined(maxVal) || modelValue <= maxVal)) {
                        modelValue = maxVal;
                    }
            ...
            ngModelCtrl.$parsers.push(function (value) {
                ...
                if (value.indexOf('-') === 0) {
                    if (minVal >= 0) {
                        changeViewValue(minVal.toString());
                        return null;
                    } else if (value === '-' || value === '-.') {
                        return null;
                    }
            ...
            if (isDefined(attrs.max) || attrs.ngMax) {
                ngModelCtrl.$validators.max = function (value) {
                    if (!(ngModelCtrl.$isEmpty(value) || isUndefined(maxVal) || value <= maxVal)) {
                        ngModelCtrl.$modelValue = maxVal;
                    }
                    return true;
                };
